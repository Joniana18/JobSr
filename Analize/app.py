...
from scraper_requests_html import scrape_all_jobs_requests_html
...
use_requests_html = st.checkbox("Use requests-html for dynamic content (slower but more reliable for JavaScript-heavy sites)")

if st.button("Scrape Jobs"):
    if url:
        st.write("Scraping data... Please wait.")
        if use_requests_html:
            job_data = scrape_all_jobs_requests_html(url, site_type)
        else:
